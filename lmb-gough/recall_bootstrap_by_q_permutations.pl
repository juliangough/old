...
$answers='/Users/gough/DropBox/Projects/clu/answers34_imputed.tsv';
...
my $perms = 1000;#5 hours on desktop mac for 1000
my $zero=0.01;
my $threshold = 0.022;
my $ratefilter=0.05;
my %termyesrates=();
...
$pval{$pheno}+=0.5;
  ...
  for my $pp (1 .. $perms){
   ...
   if ($sum/$perms < $permsave{$pheno}{$p}){
 $pvalp{$pheno}{$p}++;
}
   elsif($permsave{$pheno}{$p} == $sum/$perms){
$pvalp{$pheno}{$p}+=0.5;
}
    }
 }
 ...
 my @ppl=split /,/,$term{$pheno};
	...
	$above+=$permsave{$pheno}{$p};
	 ...
	 $pheno=$keys[$pheno];
	 $number++;
	 my $pxmean=0;	 my $pxsd=0;
	 for my $pp (1 .. $perms){
	   my $px=0;
	     foreach my $pheno2 (keys(%ns)){
	       unless (exists($pvalp{$pheno2}{$pp})){$pvalp{$pheno2}{$pp}=0;}
	       print "$pp $pheno2 $pvalue $pvalp{$pheno2}{$pp} $pval{$pheno2}\n";
	     if ($pvalp{$pheno2}{$pp} <= $pvalue){
	       $px++;
	     }
	     }
	   $pxmean+=$px;
    ...
    $pxmean=$pxmean/$perms;
      $pxsd=$pxsd/$perms-$pxmean**2;
      $pxsd=$pxsd**0.5;	 
my     $zscore;
	 if ($pxsd == 0){$zscore=0;}else{$zscore=($number-$pxmean)/$pxsd;}
	 print "Summary\t",$pvalue/$perms,"\t$number\t",scalar(keys(%ns))*$pvalue/$perms,"\t$pxmean\t$pxsd\t$zscore\n";
       }
       last;
     }
     else{
