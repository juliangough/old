...
unless ($ancestrydna == 1){
#open S,("snps.data");
open S,("/data/gough/rihab/api.23andme.com");
while (<S>){
  unless (/^#/ or /^index/){
    my    @temp=split /\t/,$_;
    chomp($temp[3]);
    $alleles{"$temp[2]:$temp[3]"}=1;
  }
}
close S;
}
...
if (scalar(@temp) == 4){
if ($ancestrydna == 1){$alleles{"$temp[1]:$temp[2]"}=1;}
...
close F;
$x++;
  print STDERR "$x of ",scalar(@files)," files read in ($badsnps/$snps bad SNPs) $file\n";
  if ($badsnps > 30169 or $snps < 50000){
    $badfiles{$file}=1;
    system ("mv '$file' badfiles/");
  }
}
}
